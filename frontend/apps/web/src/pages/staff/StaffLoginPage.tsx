...
export default function StaffLoginPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const destination = ADMIN_ROLES.includes(res.user.role) ? '/admin/dashboard' : '/admin/assistant'
  ...
  return (
    <div className="w-full max-w-md">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-xs text-text-sec mt-1">Hôpital Général de Douala · Staff Portal</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-card-lg p-8">
        <div className="text-center mb-6">
          <div className="w-10 h-10 bg-hgd-blue3 rounded-full flex items-center justify-center mx-auto mb-3">
            <ShieldCheck size={18} className="text-hgd-blue" />
          </div>
          <h1 className="text-xl font-bold text-text-pri">Staff Sign In</h1>
          <p className="text-sm text-text-sec mt-1">Authorised hospital personnel only</p>
        </div>
        ...
        <div className="mt-4 text-center">
          <Link to="/forgot-password" className="text-xs text-text-sec hover:text-hgd-blue transition-colors">
      ...
      <p className="text-center text-xs text-text-sec mt-6">
        ...
        <span className="text-hgd-blue font-medium">It runs on a separate address.</span>
      </p>
    </div>
  )
}
